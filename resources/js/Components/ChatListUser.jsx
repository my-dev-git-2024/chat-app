...
// Helper function to detect image message
const isImageMessage = (msg) => {
    return typeof msg === 'string' && msg.startsWith("storage/chat_images/");
};
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center text-xs text-gray-400">
                                        <div>
                                            {chat?.sender_id === auth.user.id && !chat?.message_deleted_at && (
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    ...
                                                    className={clsx(chat?.seen_at ? 'text-cyan-500' : 'text-gray-400/80', "w-4 h-4 mr-1")}>
                                        ...
                                        {chat?.message_deleted_at ? (
                                            ...
                                            <div className="overflow-hidden break-all" style={{ display: "-webkit-box", WebkitLineClamp: 1, WebkitBoxOrient: "vertical" }}>
                                                {isImageMessage(chat?.message) ? "📷 Photo" : chat?.message}
                                            </div>
                                        )}
                                    </div>
